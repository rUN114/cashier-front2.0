<template>
  <el-container class="layout-container">
    <el-aside width="220px">
      <div class="logo">👨‍🍳 {{ $t('role.chef') }}</div>
      <el-menu router :default-active="route.path" background-color="#304156" text-color="#bfcbd9" active-text-color="#ffd04b">
        <el-menu-item index="/chef/home"><el-icon><HomeFilled /></el-icon>{{ $t('menu.home') }}</el-menu-item>
        <el-menu-item index="/chef/orders"><el-icon><List /></el-icon>{{ $t('menu.orderQuery') }}</el-menu-item>
        <el-menu-item index="/chef/recipes"><el-icon><KnifeFork /></el-icon>{{ $t('menu.recipeManage') }}</el-menu-item>
        <el-menu-item index="/chef/consumptions"><el-icon><TrendCharts /></el-icon>{{ $t('menu.consumptionManage') }}</el-menu-item>
        <el-menu-item index="/chef/logs/changelog"><el-icon><Clock /></el-icon>{{ $t('menu.changelog') }}</el-menu-item>
      </el-menu>
    </el-aside>
    <el-container>
      <el-header class="header-bar">
        <el-breadcrumb separator="/">
          <el-breadcrumb-item>{{ $t('role.chef') }}</el-breadcrumb-item>
          <el-breadcrumb-item v-if="route.meta.title">{{ route.meta.title }}</el-breadcrumb-item>
        </el-breadcrumb>
        <div class="header-right">
          <LangSwitcher variant="light" />
          <span>{{ user.realName || user.username }} <el-tag size="small" type="info">{{ $t('role.chef') }}</el-tag></span>
