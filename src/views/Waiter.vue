<template>
  <el-container class="layout-container">
    <el-aside width="220px">
      <div class="logo">🍽️ {{ $t('role.waiter') }}</div>
      <el-menu router :default-active="route.path" background-color="#304156" text-color="#bfcbd9" active-text-color="#ffd04b">
        <el-menu-item index="/waiter/home"><el-icon><HomeFilled /></el-icon>{{ $t('menu.home') }}</el-menu-item>
        <el-menu-item index="/waiter/pos"><el-icon><ShoppingCart /></el-icon>{{ $t('menu.pos') }}</el-menu-item>
        <el-menu-item index="/waiter/orders"><el-icon><List /></el-icon>{{ $t('menu.orderQuery') }}</el-menu-item>
        <el-menu-item index="/waiter/logs/changelog"><el-icon><Clock /></el-icon>{{ $t('menu.changelog') }}</el-menu-item>
      </el-menu>
    </el-aside>
    <el-container>
      <el-header class="header-bar">
        <el-breadcrumb separator="/">
          <el-breadcrumb-item :to="{ path: '/waiter/home' }">{{ $t('role.waiter') }}</el-breadcrumb-item>
          <el-breadcrumb-item v-if="route.meta.title">{{ route.meta.title }}</el-breadcrumb-item>
        </el-breadcrumb>
        <div class="header-right">
          <LangSwitcher variant="light" />
          <span>{{ user.realName || user.username }} <el-tag size="small" type="info">{{ $t('role.waiter') }}</el-tag></span>
